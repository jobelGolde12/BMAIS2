<template>
    <div class="home">
        <div class="main-content-header container d-flex flex-row justify-content-around mt-5">
         <div class="card bg-primary">
            <div class="card-body text-light"><i class="bi bi-people me-2"></i>1,500 Resident's</div>
         </div>

         <div class="card bg-secondary">
            <div class="card-body text-light"><i class="bi bi-bell me-2"></i> 50+ Notifications</div>
         </div>

         <div class="card bg-info cursor-pointer">
            <div class="card-body text-light"><i class="bi bi-cash me-2"></i><span class="fund-span"></span> Funds</div>
         </div>
        </div>

        <div>
          <div class="container mt-5 date-container">
            <h5 class="mb-0">Funds now at <span class="current-date">02/23/2024</span> is, <span class="fund">100k</span></h5>
            <p>Funds for one year <span class="text-primary">{{ getTotalFundsForOneYear }}</span></p>
          </div>
    <div class="chart-container container-fluid">
        <canvas id="myChart" class="mt-4 container-fluid"></canvas>
    </div>
</div>

<div class="container mt-5 mt-3 ms-2">
<h5>Community Calendar</h5>
</div>
<div class="container">
  <div class="container d-flex justify-content-between align-items-center">
    <div><p class="text-dark">Explore Different Event's</p></div>
    <button class="btn btn-primary">
      <i class="bi bi-plus"></i>
      Add Events
    </button>
  </div>
  <table class="events-table table bordered-table striped-table mt-3">
    <thead>
      <tr>
        <th>Events</th>
        <th>Announced By</th>
        <th>Date</th>
        <th>Delete</th>
      </tr>
    </thead>

    <tbody>
      <tr v-for="(evtName , index) in eventsName" :key="index">
        <td>
          {{ evtName }}
        </td>
        <td class="d-flex align-items-center gap-1">
          <img src="data:image/png;base64," alt="">
          <p class="mt-0">{{ eventsAnnounce[index]}}</p>
        </td>

        <td>{{ eventsDate[index] }}</td>
        <td><button class="btn btn-warning"> <i class="bi bi-trash"></i> Delete</button>
        <span>
          <i class="bi bi-check ms-5 bg-success p-2 rounded text-light" v-if="eventStatus[index] === 'Done'">Done</i>
          <i class="bi bi-person-workspace ms-5 bg-info p-2 rounded text-light" v-if="eventStatus[index] === 'Ongoing'">Ongoing</i>
        </span>
        </td>
      </tr>
    </tbody>
  </table>

  <!-- todo meeting -->
</div>
    </div>
</template>



<script>
export default {
    data(){
        return{
            name: '',
           funds: 50000,
           fundsArr: ['50000','19000', '30000' , '35000', '20000','30000', '50000', '80000', '80000', '18000', '100000', '40000'],
           getTotalFundsForOneYear: null,
           eventsMonth: '1',
           eventsDay: '30',
           eventsYear: '2024',
           eventsName: [],
           eventsAnnounce: [],
           eventsAnnounceProfile: [],
           eventsDate: [],
           eventStatus: []
        }
    },
    methods: {
        getChart(){
            const ctx = document.getElementById('myChart');

  new Chart(ctx, {
    type: 'bar',
    data: {
      labels: ['January', 'Febuary', 'March', 'April', 'May', 'June','July','August','September','October','November','December'],
      datasets: [{
        label: 'Funds',
        data: [this.fundsArr[0], this.fundsArr[1], this.fundsArr[2], this.fundsArr[3], this.fundsArr[4], this.fundsArr[5],this.fundsArr[6],this.fundsArr[7],this.fundsArr[8],this.fundsArr[9],this.fundsArr[10],this.fundsArr[11]],
        borderWidth: 1
      }]
    },
    options: {
      scales: {
        y: {
          beginAtZero: false
        }
      }
    }
  });
        },
        getFunds(){
          let currentDate = document.querySelector('.current-date')
          let date = new Date()
          let getMonth = ''
          switch (date.getMonth()) {
              case 0:
               getMonth = 'January'
              break;
              case 1:
               getMonth = 'Febuary'
              break;
              case 0:
               getMonth = 'March'
              break;
              case 0:
               getMonth = 'April'
              break;
              case 0:
               getMonth = 'May'
              break;
              case 0:
               getMonth = 'June'
              break;
              case 0:
               getMonth = 'July'
              break;
              case 0:
               getMonth = 'August'
              break;
              case 0:
               getMonth = 'September'
              break;
              case 0:
               getMonth = 'Occtober'
              break;
              case 0:
               getMonth = 'November'
              break;
              case 0:
               getMonth = 'December'
              break;
          
          }
          currentDate.innerHTML = getMonth + ' ' + date.getDate()

          document.querySelector('.fund-span').innerHTML = this.funds
          document.querySelector('.fund').innerHTML = this.funds

        },
        addEventData(){
          this.eventsName.push("Clean-up-drive","waste segregation","Feeding program")
          this.eventsAnnounce.push("Maryjane Golde","Shyryl Villianueva","Maryjane Golde")
          this.eventsAnnounceProfile.push("data:image/png;base64,",
          "data:image/png;base64,",
          "data:image/png;base64,")
          this.eventsDate.push(`${this.eventsMonth}-${this.eventsDay}-${this.eventsYear}`,`${this.eventsMonth}-${this.eventsDay}-${this.eventsYear}`,`${this.eventsMonth}-${this.eventsDay}-${this.eventsYear}`)
          this.eventStatus.push('Done', 'Ongoing', 'Done')
       },
       getFundsForAYear(){
        let sum = 0
        for(let i = 0; i < this.fundsArr.length; i++){
         sum += +this.fundsArr[i]
        }
        this.getTotalFundsForOneYear = sum
       }
    },
    mounted(){
        this.getChart(),
        this.getFunds(),
        this.addEventData(),
        this.getFundsForAYear()
    }
}
</script>

<style lang="css">
 @import "./styles/BmaisMainContentCss.css";
</style>